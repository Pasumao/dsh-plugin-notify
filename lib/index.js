/**
 * dsh-plugin-notice — 任务结束 Windows 通知插件（纯 host 端）
 *
 ...
 *
 * @module @dsh-external/dsh-plugin-notice
 */
...
export const name = 'dsh-plugin-notice'
export const inject = []
...
function fireToast(ctx, cfg, { title, body }) {
  ...
  try {
    ...
    ctx.logger.warn(`dsh-plugin-notice: failed to launch notification: ${String(error)}`)
    return
  }
  child.on('error', (error) => {
    ctx.logger.warn(`dsh-plugin-notice: notification process error: ${String(error)}`)
  })
}
...
export function apply(ctx, config = {}) {
  ...
  if (cfg.tray !== false) {
    ...
    try {
      ...
      ctx.logger.warn(`dsh-plugin-notice: failed to start tray icon: ${String(error)}`)
    }
  }
  ...
  const notify = (id, state, kind) => {
    ...
    ctx.logger.debug(`dsh-plugin-notice: ${kind} -> "${message.title}" (session ${id})`)
  }
